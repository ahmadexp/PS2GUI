...
**IBM Easy-Setup** skin — the exact mauve desktop and DAC palette, the white icon tiles in a **5×2
grid**, the heavy condensed title lettering, the little duck — running entirely in **VGA mode 12h
(640×480×16)**.
...
The layout matches Easy-Setup exactly: a **5×2 grid** at the same tile pitch (tiles at x = 72 + col·104,
y = 150 + row·122), the five-colour DAC palette lifted pixel-for-pixel from a real Easy-Setup capture,
and the title rendered in a heavy, condensed, right-leaning face (Impact + a small oblique shear) to
mirror the "Easy-Setup" lettering. Long labels word-wrap to up to three ≤12-char lines so they never
spill into neighbouring tiles.
