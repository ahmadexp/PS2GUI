...
*Version 0.8 · by Ahmad Byagowi*

A GUI fork of [PS2TUI](https://github.com/ahmadexp/PS2TUI) that reproduces the PC110's own
**IBM Easy-Setup** — the mauve desktop, the white icon tiles with dark-red pixel-art, the stylized
title, the little duck — as a real **icon grid**, running entirely in **VGA mode 12h (640×480×16)**.

![PS2GUI — the main menu](screenshot.png)

*The main menu is an Easy-Setup icon grid: one tile per top-level category. The selected tile is
inverted (dark-red), exactly like the real Easy-Setup.*

### What changed in v0.8 — a true Easy-Setup icon grid

The menus are now **icon grids**, just like the machine's Easy-Setup:

- **Main menu** = a grid of **ten tiles**, one per PS2TUI category, each with its own pixel-art icon
  (battery, monitor, plug, keyboard, gear, chip, clipboard, magnifier, floppy, info) and its name
  below.
- **Open a tile → a new icon-grid page** for that category, one tile per item, each with an icon for
  what it does — **sliders** for a setting, **battery** for charging, **play** for an action,
  **magnifier** for a diagnostic/dump, **ⓘ** for firmware info.
...
The tile artwork — the 48×48 icons, the "PS2-GUI" title and the duck — is generated by
`tools/make_tiles.py` into `TILES.INC`. Everything (title, tiles, labels, duck, copyright) is drawn in
mode 12h on the mauve Easy-Setup desktop; there is **no text-mode menu** anywhere.

*(v0.6 removed the old launcher + PS2TUI text menu; v0.7 was a transitional list-with-icons; v0.8 is
the full Easy-Setup icon grid for both levels.)*

| Screen | |
|---|---|
| Main menu — 10 category tiles | ![main](screenshot.png) |
| A sub-menu page (Devices) | ![sub](screenshot-sub.png) |
