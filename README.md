...
the white icon tiles in a **5×2 grid**, the heavy condensed title lettering, **mouse support with the
Easy-Setup arrow cursor** — running entirely in **VGA mode 12h (640×480×16)**.
...
The tile artwork — ~50 detailed 48×48 icons, the "PS2-GUI" title and the arrow cursor — is generated by
...
If an INT 33h mouse driver is loaded (the PC110 has one), PS2GUI shows the **Easy-Setup arrow cursor**
(a hand-drawn white-with-black-outline pointer, so it stays visible on any tile). **Left-click** a tile
to open it, an item to pick its value, or a value to choose it; **right-click** goes back one level.
Without a mouse driver everything still works from the keyboard.
